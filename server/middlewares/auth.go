...
type LogIn struct {
	...
	SigOrTx       []byte `json:"sigOrTx" binding:"required"`
}
...
func AuthMiddleware() (authMiddleware *jwt.GinJWTMiddleware) {
	authMiddleware, err := jwt.New(&jwt.GinJWTMiddleware{
		...
		IdentityHandler: func(c *gin.Context) interface{} {
			...
			var userID = uint(claims["id"].(float64))
			banned := utils.CheckBannedUser(userID)
		...
		Authenticator: func(c *gin.Context) (interface{}, error) {
			...
			if result := db.Preload("Wallet.Balance.ChipBalance").Where("wallet_address = ?", walletAddress).First(&user); result.Error != nil {
			...
			if err != nil {
				...
				log.LogMessage("Auth User", "Success", "info", logrus.Fields{"UserID": user.ID, "Nonse": user.Nonce})
			...
			if len(tx.Message.Instructions) == 0 {
				return nil, errors.New("failed to unmarshal solana transaction")
			}
